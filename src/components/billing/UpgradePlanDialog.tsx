          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl w-full">
            {availablePlans
              .filter((plan) => {
                ...
                if (isFreePlan(plan) && currentPlan && isFreePlan(currentPlan)) {
                  return false;
                }
                return true;
              })
              ...
              const canSelectExpiredPlan = planDetails?.isExpired && plan.id === currentPlan?.id;
              ...
              return (
                ...
                >
                  ...
                  {isCurrentPlan && (
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-10">
                      <Badge className="bg-gradient-to-r from-gray-500 to-gray-600 text-white px-4 py-1.5 text-sm font-medium shadow-lg">
                        {planDetails?.isExpired ? "Your expired plan" : "Your current plan"}
                      </Badge>
                    </div>
                  )}
                  ...
                  <div className="p-8 text-center">
                    ...
                    <Button
                      onClick={() =>
                        (isCurrentPlan && !canSelectExpiredPlan) ? null : handleUpgrade(plan)
                      }
                      disabled={isCurrentPlan && !canSelectExpiredPlan}
                      className={cn(
