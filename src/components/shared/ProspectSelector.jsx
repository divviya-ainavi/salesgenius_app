        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-medium text-sm">{selectedProspect.companyName}</h3>
                <p className="text-xs text-muted-foreground">
                  {selectedProspect.prospectName} • {selectedProspect.title}
                </p>
              </div>
              <Badge variant="outline" className={cn("text-xs", getStatusColor(selectedProspect.status))}>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{selectedProspect.lastCallDate}</span>
              </div>
              <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{selectedProspect.probability}%</span>
              </div>
              <div className="flex items-center space-x-2">
                <User className="w-3 h-3 text-muted-foreground" />
                <span className="text-muted-foreground">Stakeholders:</span>
                <span className="font-medium">{selectedProspect.stakeholders?.length || 0}</span>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-primary/20">
              <p className="text-xs text-muted-foreground">
                <span className="font-medium">Next Action:</span> {selectedProspect.nextAction}
              </p>
            </div>
          </div>
        )}
            ...
            {filteredProspects.map((prospect) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h4 className="font-medium text-sm">{prospect.companyName}</h4>
                    <p className="text-xs text-muted-foreground">
