...
export const dbHelpers = {
  ...
  // Get call insights for a user
  async getCallInsights(userId) {
    try {
      const { data, error } = await supabase
        .from('call_insights')
        .select('*')
  ...
  // Get insights for a user (from insights table)
  ...
  // Get email prospect insights for a user
  ...
  // Get prospect data (updated function name for consistency)
  ...
  async getActionItemsByProspectId(prospectId) {
    try {
      const { data, error } = await supabase
        .from('action_items')
        .select('*')
        .eq('people_id', prospectId); // Assuming action_items are linked to people

      if (error) throw error;
      return data || [];
    } catch (error) {
      console.error('Error fetching action items by prospect ID:', error);
      throw error;
    }
  },

  // Process call transcript
  async processCallTranscript(data) {
    try {
      // This would typically call your AI processing API
      // For now, we'll create a basic insight record
      const { data: insight, error } = await supabase
        .from('insights')
        .insert([{
          user_id: data.userId,
          extracted_transcript: data.transcript,
          processing_status: 'completed',
          type: 'call_processing',
          fireflies_id: data.metadata?.firefliesId,
          call_summary: `Processed call with ${data.metadata?.companyName}`,
        }])
        .select()
        .single();
