...
export const dbHelpers = {
  ...
  // Personal Insights functions
  async getPersonalInsightsFiles(userId) {
    const { data, error } = await supabase
      .from('business_knowledge_files')
      .select('*')
  ...
  async savePersonalInsights(userId, organizationId, insightsData) {
    // Create a new table entry for personal insights
    const { data, error } = await supabase
      .from('personal_insights')
      .insert([{
        user_id: userId,
        organization_id: organizationId,
        rep_name: insightsData.repName,
        role_title: insightsData.roleTitle,
        territory: insightsData.territory,
        vertical_focus: insightsData.verticalFocus,
        quota: insightsData.quota,
        time_horizon: insightsData.timeHorizon,
        active_pipeline: insightsData.activePipeline,
        personal_proof_bank: insightsData.personalProofBank,
        relationship_capital: insightsData.relationshipCapital,
        selling_style_strengths: insightsData.sellingStyleStrengths,
        common_objections_encountered: insightsData.commonObjectionsEncountered,
        preferred_advance_per_account: insightsData.preferredAdvancePerAccount,
        availability_windows: insightsData.availabilityWindows,
        product_certifications: insightsData.productCertifications,
        brand_voice_tone: insightsData.brandVoiceTone,
        summary_note: insightsData.summaryNote,
  ...
  async updatePersonalInsights(insightsId, updatedData) {
    const { data, error } = await supabase
      .from('personal_insights')
      .update({
  ...
  async deletePersonalInsightsFile(fileId) {
