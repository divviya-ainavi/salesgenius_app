...
const formatResearchDataForCopy = (data) => {
  if (!data) return "No research data available";

  const sections = [];
  
  // Header
  sections.push(`COMPANY RESEARCH REPORT`);
  sections.push(`Company: ${data.company_name || 'Unknown'}`);
  ...
  if (data.executive_summary) {
    ...
    if (data.executive_summary.key_highlights) {
      ...
      data.executive_summary.key_highlights.forEach((highlight, index) => {
        sections.push(`${index + 1}. ${highlight}`);
      });
      sections.push('');
    }
    
    if (data.executive_summary.strategic_focus_areas) {
      sections.push(`Strategic Focus Areas:`);
      data.executive_summary.strategic_focus_areas.forEach((area, index) => {
        sections.push(`${index + 1}. ${area}`);
      });
      sections.push('');
    }
    
    if (data.executive_summary.summary) {
      sections.push(`Summary:`);
      sections.push(data.executive_summary.summary);
      sections.push('');
    }
  ...
  if (data.company_overview) {
    ...
    sections.push(`${'-'.repeat(30)}`);
    sections.push(data.company_overview);
    sections.push('');
  }

  // Company Deep Dive (Detailed Analysis)
  if (data.company_analysis_details) {
    sections.push(`COMPANY DEEP DIVE`);
    sections.push(`${'-'.repeat(30)}`);
    
    if (data.company_analysis_details.nature_of_business) {
      sections.push(`Nature of Business:`);
      sections.push(data.company_analysis_details.nature_of_business);
      sections.push('');
    }
    
    if (data.company_analysis_details.geographic_scope) {
      sections.push(`Geographic Scope:`);
      sections.push(data.company_analysis_details.geographic_scope);
      sections.push('');
    }
    
    if (data.company_analysis_details.size) {
      sections.push(`Company Size:`);
      sections.push(data.company_analysis_details.size);
      sections.push('');
    }
    
    if (data.company_analysis_details.sector) {
      sections.push(`Sector:`);
      sections.push(data.company_analysis_details.sector);
      sections.push('');
    }
    
    if (data.company_analysis_details.key_insights) {
      sections.push(`Key Insights:`);
      data.company_analysis_details.key_insights.forEach((insight, index) => {
        sections.push(`${index + 1}. ${insight}`);
      });
      sections.push('');
    }
    
    if (data.company_analysis_details.business_model) {
      sections.push(`Business Model:`);
      sections.push(data.company_analysis_details.business_model);
      sections.push('');
    }
    
    if (data.company_analysis_details.revenue_streams) {
      sections.push(`Revenue Streams:`);
      data.company_analysis_details.revenue_streams.forEach((stream, index) => {
        sections.push(`${index + 1}. ${stream}`);
      });
      sections.push('');
    }
  ...
  if (data.market_analysis) {
    sections.push(`MARKET ANALYSIS & GROWTH OPPORTUNITIES`);
    sections.push(`${'-'.repeat(30)}`);
    
    if (data.market_analysis.market_position) {
      sections.push(`Market Position:`);
      sections.push(data.market_analysis.market_position);
      sections.push('');
    }
    
    if (data.market_analysis.competitive_landscape) {
      sections.push(`Competitive Landscape:`);
      sections.push(data.market_analysis.competitive_landscape);
      sections.push('');
    }
    
    if (data.market_analysis.market_trends) {
      sections.push(`Market Trends:`);
      data.market_analysis.market_trends.forEach((trend, index) => {
        sections.push(`${index + 1}. ${trend}`);
      });
      sections.push('');
    }
    
    if (data.market_analysis.growth_opportunities) {
      sections.push(`Growth Opportunities:`);
      data.market_analysis.growth_opportunities.forEach((opportunity, index) => {
        sections.push(`${index + 1}. ${opportunity}`);
      });
      sections.push('');
    }
  ...
  if (data.demand_side) {
    sections.push(`DEMAND SIDE: PROSPECT'S NEEDS`);
    sections.push(`${'-'.repeat(30)}`);
    
    if (data.demand_side.pain_points) {
      sections.push(`Pain Points:`);
      data.demand_side.pain_points.forEach((pain, index) => {
        sections.push(`${index + 1}. ${pain}`);
      });
      sections.push('');
    }
    
    if (data.demand_side.needs_analysis) {
      sections.push(`Needs Analysis:`);
      sections.push(data.demand_side.needs_analysis);
      sections.push('');
    }
    
    if (data.demand_side.buying_criteria) {
      sections.push(`Buying Criteria:`);
      data.demand_side.buying_criteria.forEach((criteria, index) => {
        sections.push(`${index + 1}. ${criteria}`);
      });
      sections.push('');
    }
    
    if (data.demand_side.decision_factors) {
      sections.push(`Decision Factors:`);
      data.demand_side.decision_factors.forEach((factor, index) => {
  ...
  if (data.profiles && Array.isArray(data.profiles)) {
    sections.push(`STAKEHOLDER PROFILES`);
    sections.push(`${'-'.repeat(30)}`);
    
    data.profiles.forEach((profile, index) => {
      sections.push(`${index + 1}. ${profile}`);
    });
    sections.push('');
  }

  // Company Analysis (Legacy format support)
  if (data.company_analysis) {
    sections.push(`COMPANY ANALYSIS`);
    sections.push(`${'-'.repeat(30)}`);
    sections.push(data.company_analysis);
    sections.push('');
  }
  ...
  if (data.recommendations) {
    ...
    sections.push(`${'-'.repeat(30)}`);
    sections.push(data.recommendations);
    sections.push('');
  }

  // Market Trends (if separate from market_analysis)
  ...
  // Growth Opportunities (if separate from market_analysis)
  if (data.growth_opportunities && Array.isArray(data.growth_opportunities)) {
    sections.push(`GROWTH OPPORTUNITIES`);
    sections.push(`${'-'.repeat(30)}`);
  ...
  if (data.key_positioning) {
    sections.push(`KEY POSITIONING`);
    sections.push(`${'-'.repeat(30)}`);
    sections.push(data.key_positioning);
    sections.push('');
  }
  ...
  if (data.sources && Array.isArray(data.sources)) {
    ...
    data.sources.forEach((source, index) => {
      if (typeof source === 'string') {
        ...
        sections.push(`${index + 1}. ${source.title || source.name || 'Untitled Source'}`);
        if (source.url) {
          sections.push(`   URL: ${source.url}`);
        }
        if (source.description) {
          sections.push(`   Description: ${source.description}`);
        }
      }
    });
    sections.push('');
  ...
  if (data.summary_note) {
    ...
    sections.push(`${'-'.repeat(30)}`);
    sections.push(data.summary_note);
    sections.push('');
  }
