...
• Prepare questions that demonstrate understanding of their industry challenges`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.prospectFile) {
        try {
          const uploadResult = await fileStorage.uploadFile(
            formData.prospectFile,
            user?.id
          );
          uploadedFileId = uploadResult.filePath;
          ...
          const { data: fileRecord, error: fileError } = await supabase
            ...
            .insert([{
              ...
              file_type: 'pdf',
              file_size: formData.prospectFile.size,
              content_type: formData.prospectFile.type,
      ...
      const response = await fetch(
        `${config.api.baseUrl}${config.api.endpoints.companyResearch}`,
        {
          ...
          body: JSON.stringify({
            ...
            prospectFileId: uploadedFileId,
            prospectFileUrl: uploadedFileUrl,
          }),
        }
      );
      ...
      await dbHelpers.saveResearchCompany({
        ...
        prospect_file_id: uploadedFileId,
        prospect_urls: uploadedFileUrl ? [uploadedFileUrl] : [],
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Prospect File Upload */}
                <div className="space-y-2">
                  <label className="text-sm font-medium">
                    Prospect Information (PDF)
                  </label>
                  <FileUpload
                    onFileSelect={handleFileSelect}
                    selectedFile={formData.prospectFile}
                    accept={{
                      'application/pdf': ['.pdf']
                    }}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Optional: Upload a PDF with prospect information for personalized analysis
                  </p>
                </div>
