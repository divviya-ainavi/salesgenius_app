        ...
        {isLoadingHistory ? (
          ...
          <div className="grid gap-3">
            {filteredHistory.map((item) => {
              const parsedProfiles = parseProspectProfiles(item.prospectProfiles || item.profiles);
              ...
              return (
                ...
                >
                  <CardContent className="p-4">
                    {/* Header Row - Company Name and Date */}
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                          <Building className="w-4 h-4 text-blue-600" />
                        </div>
                        <h4 className="font-semibold text-base truncate">{item.company_name}</h4>
                      </div>
                    ...
                    <p className="text-sm text-muted-foreground line-clamp-2 mb-3 leading-relaxed">
                      {item.summary_note || item.company_analysis?.substring(0, 120) + "..." || "No summary available"}
                    </p>
                      ...
                      <div className="flex items-center space-x-4">
                        {parsedProfiles.length > 0 && (
                          <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                            ...
                            <span>{parsedProfiles.length} profile{parsedProfiles.length !== 1 ? 's' : ''}</span>
                          </div>
                        )}
                        {item.market_trends && item.market_trends.length > 0 && (
                          <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                            <TrendingUp className="w-3 h-3" />
                            <span>{item.market_trends.length} trend{item.market_trends.length !== 1 ? 's' : ''}</span>
                          </div>
                        )}
                        {item.growth_opportunities && item.growth_opportunities.length > 0 && (
                          <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                            <Target className="w-3 h-3" />
                            <span>{item.growth_opportunities.length} opportunit{item.growth_opportunities.length !== 1 ? 'ies' : 'y'}</span>
                          </div>
                        )}
                      </div>
