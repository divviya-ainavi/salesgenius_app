  ...
  const handleCopy = (section) => {
    ...
    switch (section) {
      case "executiveSummary":
        if (researchResult?.executiveSummary) {
          textToCopy = `Executive Summary:\n\n${researchResult.executiveSummary}`;
        }
        break;

      case "companyOverview":
        if (researchResult?.companyOverview) {
          textToCopy = `Company Overview:\n\n${researchResult.companyOverview}`;
        }
        break;

      case "demandSide":
        if (researchResult?.demandSide) {
          textToCopy = "Demand Side Analysis:\n\n";
          
          // Static Elements
          if (researchResult.demandSide.staticElements) {
            textToCopy += "STATIC DEMAND ELEMENTS:\n\n";
            
            if (researchResult.demandSide.staticElements.industryRequirements?.length > 0) {
              textToCopy += "Industry Requirements:\n";
              researchResult.demandSide.staticElements.industryRequirements.forEach((req, index) => {
                textToCopy += `• ${req}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.staticElements.operationalNeeds?.length > 0) {
              textToCopy += "Operational Needs:\n";
              researchResult.demandSide.staticElements.operationalNeeds.forEach((need, index) => {
                textToCopy += `• ${need}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.staticElements.recurringProblems?.length > 0) {
              textToCopy += "Recurring Problems:\n";
              researchResult.demandSide.staticElements.recurringProblems.forEach((problem, index) => {
                textToCopy += `• ${problem}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.staticElements.geoSizeRequirements?.length > 0) {
              textToCopy += "Geographic & Size Requirements:\n";
              researchResult.demandSide.staticElements.geoSizeRequirements.forEach((req, index) => {
                textToCopy += `• ${req}\n`;
              });
              textToCopy += "\n";
            }
          ...
          // Dynamic Elements
          if (researchResult.demandSide.dynamicElements) {
            textToCopy += "DYNAMIC DEMAND ELEMENTS:\n\n";
            
            if (researchResult.demandSide.dynamicElements.marketPressures?.length > 0) {
              textToCopy += "Market Pressures:\n";
              researchResult.demandSide.dynamicElements.marketPressures.forEach((pressure, index) => {
                textToCopy += `• ${pressure}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.dynamicElements.technologyDisruptions?.length > 0) {
              textToCopy += "Technology Disruptions:\n";
              researchResult.demandSide.dynamicElements.technologyDisruptions.forEach((disruption, index) => {
                textToCopy += `• ${disruption}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.dynamicElements.competitiveThreats?.length > 0) {
              textToCopy += "Competitive Threats:\n";
              researchResult.demandSide.dynamicElements.competitiveThreats.forEach((threat, index) => {
                textToCopy += `• ${threat}\n`;
              });
              textToCopy += "\n";
            }
            
            if (researchResult.demandSide.dynamicElements.growthPhaseDemands?.length > 0) {
              textToCopy += "Growth Phase Demands:\n";
              researchResult.demandSide.dynamicElements.growthPhaseDemands.forEach((demand, index) => {
                textToCopy += `• ${demand}\n`;
              });
            }
          }
...
`.trim();
        ...
        textToCopy = researchResult.is_new
