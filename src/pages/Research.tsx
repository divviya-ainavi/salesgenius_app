...
const Research = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Now upload files to storage after successful API response
      const uploadedFileData = [];
      ...
      for (const fileItem of uploadedFiles) {
        try {
          // Upload to Supabase Storage
          const fileData = await fileStorage.uploadFile(fileItem.file, user?.id);
          ...
          if (fileError) {
            console.error("Error saving file metadata:", fileError);
            throw new Error(`Failed to save file metadata for ${fileItem.file.name}`);
          }
          ...
          uploadedFilePaths.push(fileData.filePath);
          uploadedFileUrls.push(fileData.publicUrl);
        } catch (uploadError) {
          console.error("Error uploading file:", uploadError);
          throw new Error(
                ...
                <CardContent className="space-y-3">
                  <div className="space-y-2">
                    ...
                    {research.prospect_urls &&
                        ...
                        {research.prospect_profiles && research.prospect_profiles.length > 0 && (
                          <div className="flex items-center space-x-2 text-sm">
                            <User className="w-4 h-4 text-muted-foreground" />
                            <span className="text-muted-foreground">
                              {research.prospect_profiles.length} prospect profile{research.prospect_profiles.length > 1 ? 's' : ''} analyzed
                            </span>
                          </div>
                        )}
                    ...
                    <div className="flex items-center space-x-2 text-sm">
                      ...
                      {research.prospect_profiles && research.prospect_profiles.length > 0 && (
                        <div className="pt-2 border-t border-border">
                          <h5 className="text-xs font-medium text-muted-foreground mb-1">Prospect Profiles:</h5>
                          <div className="space-y-1">
                            {research.prospect_profiles.slice(0, 2).map((profile, index) => (
                            ...
                            {research.prospect_profiles.length > 2 && (
                              <p className="text-xs text-muted-foreground">
                                +{research.prospect_profiles.length - 2} more...
                              </p>
                            )}
                          </div>
                        </div>
              ...
              {researchResult?.prospectProfiles && researchResult.prospectProfiles.length > 0 ? (
                ...
                <Card>
                  <CardContent className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium mb-2">No Prospect Profiles</h3>
                    <p className="text-muted-foreground mb-4">
                      Upload PDF files containing prospect information to generate detailed profiles and communication insights.
                    </p>
                  </CardContent>
                </Card>
              )}
