...
const Research = () => {
  ...
  const isPlanExpired = planDetails?.isExpired;
  ...
  const handleResearchClick = () => {
    ...
    // Continue with normal research functionality
    // Add your existing research logic here
    console.log("Research functionality - plan is active");
  };
  ...
  const [researchToDelete, setResearchToDelete] =
    useState<StoredResearch | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq("id", researchToDelete.id);

      if (error) throw error;

      // Remove from local state immediately
      setResearchHistory((prev) =>
        prev.filter((item) => item.id !== researchToDelete.id)
      );
  ...
  const handleDeleteClick = (research: StoredResearch, e: React.MouseEvent) => {
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to delete the research for "
                {researchToDelete?.company_name}"? This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            AI-powered company research and prospect insights
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-center">
        <button
          ...
          className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-8 py-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-3"
        >
          <Search className="w-6 h-6" />
          <span className="text-lg font-semibold">Start Company Research</span>
        </button>
      </div>
