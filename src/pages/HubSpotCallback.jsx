...
const HubSpotCallback = () => {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (error) {
        setStatus('error');
        setMessage(`HubSpot connection failed: ${error}${errorDescription ? ` - ${errorDescription}` : ''}`);
        ...
        <CardContent className="space-y-4">
          ...
          {accountInfo && status === 'success' && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <h4 className="font-medium text-green-900 mb-2">Account Connected</h4>
              <div className="text-sm text-green-700 space-y-1">
                <p><strong>Account:</strong> {accountInfo.name}</p>
                <p><strong>Hub ID:</strong> {accountInfo.hubId}</p>
              </div>
            </div>
          )}
          ...
          <div className="text-center">
            <p className="text-xs text-muted-foreground">
              {status === 'success' && 'You will be redirected automatically in a few seconds.'}
              {status === 'processing' && 'This may take a few moments...'}
              {status === 'error' && 'Please try again or contact support if the issue persists.'}
            </p>
          </div>
        </CardContent>
      </Card>
