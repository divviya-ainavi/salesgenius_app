...
const CompleteSignup = () => {
  ...
  useEffect(() => {
    ...
    const validateSignupSession = async () => {
      try {
        ...
        // Method 3: Handle single token verification (both signup and email confirmation)
        if (urlParams.token && (urlParams.type === "signup" || urlParams.type === "email")) {
          console.log("🔄 CompleteSignup - Verifying OTP token...");

          const { data, error: verifyError } = await supabase.auth.verifyOtp({
            token_hash: urlParams.token,
            type: urlParams.type === "email" ? "email" : "signup",
          });
        ...
        // Method 4: Handle email confirmation type
        if (urlParams.type === "email_change" || urlParams.type === "signup") {
          console.log("🔄 CompleteSignup - Handling email confirmation...");
          
          // Check if we have a valid session after email confirmation
          const { data: { session }, error: sessionError } = await supabase.auth.getSession();
          
          if (session && !sessionError) {
            console.log("✅ CompleteSignup - Email confirmed, session available");
            if (isMounted) {
              setIsValidSession(true);
              setUserEmail(session.user.email);
              setIsValidating(false);
  ...
  // Alternative method: Check for email parameter in URL for development
  useEffect(() => {
    const urlParams = parseUrlParameters();
    
    // For development: if email is passed directly in URL, use it
    if (urlParams.email && !isValidSession && !isValidating) {
      console.log("🔧 Development mode - using email from URL:", urlParams.email);
      setIsValidSession(true);
      setUserEmail(urlParams.email);
      setIsValidating(false);
    }
  }, [parseUrlParameters, isValidSession, isValidating]);
