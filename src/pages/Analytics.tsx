...
const Analytics = () => {
  ...
  // Load real counts for Super Admin
  const loadRealCounts = async () => {
    setRealCounts(prev => ({ ...prev, isLoading: true }));
    try {
      // Get active users count (status_id = 1 means active)
      ...
      setRealCounts(prev => ({ ...prev, isLoading: false }));
    }
  };
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {realCounts.isLoading ? (
                    <RefreshCw className="w-6 h-6 animate-spin" />
                  ) : (
                    realCounts.activeUsers.toLocaleString()
                  )}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {realCounts.isLoading ? (
                    ...
                    realCounts.totalOrganizations.toLocaleString()
                  )}
                  %
                </p>
              </div>
