...
1. Storage Bucket
   - Creates `call-transcripts` bucket for file uploads
   - Configures public access, file size limits, and allowed MIME types
   - Handles existing bucket gracefully with upsert logic
...
3. Notes
   - Uses direct INSERT with ON CONFLICT for bucket creation
   - Conditional policy creation to avoid conflicts
   - Supports various file types including transcripts, audio, and video
*/

-- Create the storage bucket directly with INSERT and ON CONFLICT
...
-- Create storage policies using conditional logic to avoid conflicts
