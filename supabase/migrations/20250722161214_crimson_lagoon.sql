/*
  # Create Business Knowledge Storage System
  ...
  2. Storage Bucket
    - Create `business-knowledge` bucket with proper policies
...
-- Create storage bucket for business knowledge files (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'business-knowledge'
  ) THEN
...
-- Create business knowledge files table
...
-- Create trigger function for updated_at (only if it doesn't exist)
