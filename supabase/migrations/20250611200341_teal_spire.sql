/*
  ...
  1. New Tables
    - `profiles` - User profile information with HubSpot integration
    - `call_notes` - Call transcripts and AI-generated summaries
    - `call_commitments` - Action items and commitments from calls
    - `follow_up_emails` - Generated follow-up email content
    - `deck_prompts` - Presentation prompts and content
    - `push_log` - Log of data pushed to external systems
...
-- Create policies for call_notes (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for call_commitments (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for follow_up_emails (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for deck_prompts (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for push_log (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
